...
function wp_praiser_widget_init()
{
	...
	function wp_praiser_widget() {
		...
		if($praises = wp_praiser_display_praises($title, $random_order, $min_height, $refresh_interval, $show_source, $show_author, $tags, $char_limit, $char_size)) {
			echo $before_widget;
	...
	function wp_praiser_full() {
		...
		$full_page = 1;
		$page_exclude = "";
		if($praises = wp_praiser_display_full($title, $random_order, $min_height, $refresh_interval, $show_source, $show_author, $tags, $char_limit, $char_size, $full_page, $page_exclude)) {
			echo $before_widget;
			if($title) echo $before_title . $title . $after_title . "\n";
			echo $praises;
			echo $after_widget;
